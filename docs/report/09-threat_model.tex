...
\begin{itemize}
    ...
    \item \textbf{External Attack Surface}: Composed of the public REST API endpoints defined in the OpenAPI specification.
...
\paragraph{External Attack Surfaces (no JWT required)}
\begin{itemize}
    \item \api{/login}
    \item \api{/register}
\end{itemize}
...
\begin{itemize}
    ...
    \item \api{/internal/players/friendship/validation}
    \item \api{/internal/matches/create}
    \item \api{/internal/cards/validation}
\end{itemize}
...
\begin{itemize}
    ...
    Malicious payloads injected into input fields (e.g., username in \api{/players/search}) manipulate the backend database queries, potentially corrupting data (Affects \textbf{A3}, \textbf{A4}, \textbf{A7}; Crosses \textbf{TB3}).
\end{itemize}
