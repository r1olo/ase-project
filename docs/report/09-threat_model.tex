...
\begin{itemize}
    ...
    Service certificates and keys, and JWT signing keys.
...
\begin{itemize}
    \item \textbf{Network Attack Surface}: Consists of the API Gateway (Nginx), it serves as the entry point for all external traffic.
...
\begin{itemize}
    ...
    An attacker steals a valid access token (via XSS) gaining access to authenticated endpoints like \texttt{/players/me} as the victim (Affects \textbf{A1}; Crosses \textbf{TB1}).
\end{itemize}
...
\begin{itemize}
    ...
    An attacker uses a botnet to flood the \texttt{POST /enqueue} endpoint with valid tokens, filling the Redis queue and preventing legitimate players from finding a match (Affects \textbf{A3}, \textbf{A5}; Crosses \textbf{TB2}).
